...
class SecondWindow(QWidget):
    def __init__(self, width, height, problem, method):
        ...
        if self.method == "Diskriminant":
            self.canvases = self.problem.Discriminant()   

        elif self.method == "Rozklad na korene":
            self.canvases = self.problem.Factoring() 

        elif self.method == "Štvorec":
...
if __name__ == '__main__':
    ...
    problem = Equation("Úplná")
    problem.equation_genr(False)

    app = QApplication(sys.argv)

    # Example instantiation of SecondWindow
    window = SecondWindow(int(m.width*0.6),int(m.height*0.8), problem, "Štvorec")
    window.show()
    sys.exit(app.exec_())